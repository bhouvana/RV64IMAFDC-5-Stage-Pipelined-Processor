...
// A). Pipeline-level directed test for BRANCH_PREDICTOR=PREDICTOR_TOURNAMENT
// (3), using sim/programs/branch_predict_history.s's strictly-alternating
// branch pattern (see that file's own header). Architectural correctness
// only -- misprediction timing under Tournament genuinely differs from
// BHT+BTB/GShare alone by design and is not itself a pass/fail criterion
// here.
module tb_branch_predictor_tournament;
    reg clk = 0;
    reg start = 0;

    PIPELINED #(.INIT_FILE("sim/programs/branch_predict_history.mem"), .BRANCH_PREDICTOR(3))
    ...
    initial begin
        ...
        report("branch_predictor_tournament");
        $finish;
    end
endmodule
